...
int main(void)
{
	...
	try {
		...
		// build the program
		if (!build_program(&program, &context, "gaussian.cl"))
		{
		...
		std::cout << "Part i enqueued." << std::endl;
		std::cout << "--------------------" << std::endl;
		...
		write_BMP_RGBA_to_RGB("A2T3b(i)_Output.bmp", outputImage, imgWidth, imgHeight);


		//--------------------------------------------------------------------------------------------------------------
		// Part ii
		// build the program
		if (!build_program(&program, &context, "gaussianparallel.cl"))
		{
			// if OpenCL program build error
			quit_program("OpenCL program build error.");
		}
		//Create kernel
		kernel = cl::Kernel(program, "gaussianHori");
		...
		std::cout << "Gaussian (Horizontal) enqueued." << std::endl;
		std::cout << "--------------------" << std::endl;

		//Create kernel
		kernel = cl::Kernel(program, "gaussianVert");
		...
		std::cout << "Gaussian (Vertical) enqueued." << std::endl;
		std::cout << "--------------------" << std::endl;

		queue.enqueueReadImage(outputImgBuffer, CL_TRUE, origin, region, 0, 0, outputImage);

		// output results to image file
		write_BMP_RGBA_to_RGB("A2T3b(ii)_Output.bmp", outputImage, imgWidth, imgHeight);
